...
def transcribe_audio_file(file_path, diarization_results):
    ...
    for segment in diarization_results:
        ...
        # 오디오 조각 텍스트 변환
        print(
            f"Transcribing {speaker} from {segment['start']} to {segment['stop']} seconds..."
        )
...
def transcribe_audio(file_path, diarization_excel_path):
    """주어진 파일 경로와 다이어리제이션 엑셀 경로로 음성을 텍스트로 변환"""
    return process_audio(file_path, diarization_excel_path)
